...
- **July 2026 (Phase 4I - Process Section dontboardme Diagonal Card Stack & Staggered Rise Animation)** ✅: Transformed "Cara Kerja Sebooth" (`ProcessPinningSection.tsx`) to match the exact diagonal overlapping card stack UI layout and scroll animation of `dontboardme.com`. Key changes: (1) Arranged cards in an overlapping diagonal staggered layout (`-space-x-8` to `-space-x-16` with custom rotations `-6deg`, `2deg`, `-3deg`, `5deg`); (2) Added circular step badges (`1`, `2`, `3`, `4`) attached to each card header; (3) Added bottom pastel vector illustration containers (`#eef2ff` / `#fff0eb`); (4) Built GSAP ScrollTrigger timeline animation where each card rises and settles one-by-one (`fromTo({ y: 350, opacity: 0, scale: 0.8 }, { y: 0, opacity: 1, scale: 1 })`) as the user scrolls.
