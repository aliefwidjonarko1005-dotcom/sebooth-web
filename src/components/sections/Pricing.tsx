    ...
    return (
        <section id="pricing" className="py-24 px-6 md:px-16 bg-transparent relative overflow-hidden select-none">
            ...
            >
                ...
                <EditableText section="pricing" fieldKey="section_title" defaultValue={content.section_title} as="h2" className="h2 text-[#002366] font-bayon uppercase leading-none">
                    {content.section_title}
                </EditableText>
                <EditableText section="pricing" fieldKey="section_subtitle" defaultValue={content.section_subtitle} as="p" className="text-base md:text-xl font-medium text-[#ff4500] uppercase mt-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto relative z-10">
                ...
                >
                    {content.unlimited_badge && (
                        <div className="absolute -top-4 right-8 bg-[#ff4500] text-white px-5 py-2 font-black uppercase text-xs rounded-full shadow-md tracking-wider">
                            {content.unlimited_badge}
                        </div>
                    )}
                    <div>
                        <EditableText section="pricing" fieldKey="unlimited_title" defaultValue={content.unlimited_title} as="h3" className="h3 text-[#002366] font-bayon uppercase mb-1 leading-none">
                        ...
                        <ul className="space-y-3 mb-8">
                            {unlimitedFeatures.map((feature, i) => (
                                <li key={i} className="flex items-center gap-3 text-gray-800 font-bold uppercase text-xs md:text-sm bg-gray-50 p-3 rounded-xl border border-gray-100">
                                    <CheckCircle className="w-5 h-5 text-[#002366] shrink-0" />
                        ...
                        <div className="border-t border-gray-200 pt-4 mb-8 space-y-2">
                            {unlimitedPackages.map((item, index) => (
                                <div key={index} className="flex justify-between items-center py-2.5 px-4 rounded-xl hover:bg-gray-50 transition-colors">
                                    ...
                                    <EditableArrayItemText section="pricing" arrayKey="unlimited_packages" items={unlimitedPackages} index={index} field="price" as="span" className="text-[#002366] font-black text-base md:text-lg font-bayon" />
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <Link href={`${waBase}${encodeURIComponent(content.unlimited_wa_text)}`} target="_blank"
                        className="block w-full bg-[#002366] text-white font-black uppercase py-4 rounded-2xl hover:bg-[#00113a] hover:scale-[1.02] active:scale-95 transition-all text-center tracking-wider shadow-md text-sm md:text-base">
                ...
                >
                    <div>
                        <EditableText section="pricing" fieldKey="quota_title" defaultValue={content.quota_title} as="h3" className="h3 text-[#ff4500] font-bayon uppercase mb-1 leading-none">
                            {content.quota_title}
                        </EditableText>
                        ...
                        <div className="border-t border-gray-200 pt-4 mb-8 space-y-2">
                            {quotaPackages.map((item, index) => (
                                <div key={index} className="flex justify-between items-center py-2.5 px-4 rounded-xl hover:bg-gray-50 transition-colors">
                                    ...
                                    <EditableArrayItemText section="pricing" arrayKey="quota_packages" items={quotaPackages} index={index} field="price" as="span" className="text-[#ff4500] font-black text-base md:text-lg font-bayon" />
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <Link href={`${waBase}${encodeURIComponent(content.quota_wa_text)}`} target="_blank"
                        className="block w-full bg-[#ff4500] text-white font-black uppercase py-4 rounded-2xl hover:bg-[#e03d00] hover:scale-[1.02] active:scale-95 transition-all text-center tracking-wider shadow-md text-sm md:text-base">
