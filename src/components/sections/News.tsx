...
export function News({ initialNews = [] }: NewsProps) {
    ...
    return (
        <section id="news" className="py-24 px-6 md:px-20 bg-transparent paper-texture">
            {/* Header */}
                ...
                {initialNews.map((item, idx) => (
                    <Link key={item.id} href={`/news/${item.id}`} className="block group">
                        <motion.article
                            ...
                            className={`bg-white border-2 border-black overflow-hidden group-hover:-translate-y-1 group-hover:-translate-x-1 transition-all duration-200 ${shadowCycle[idx % shadowCycle.length]}`}
                        >
                            {/* Image */}
                            {item.image_url && (
                            ...
                            <div className="p-6">
                                ...
                                <span className="inline-block mt-4 text-sm font-black text-secondary uppercase">
                                    Baca Selengkapnya →
                                </span>
                            </div>
                        </motion.article>
