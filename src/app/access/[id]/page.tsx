...
import { Camera, Download, LayoutGrid, Loader2, CheckCircle, XCircle } from 'lucide-react'
  ...
  useEffect(() => {
    async function fetchSession() {
      ...
      if (error) {
        ...
        if (data.is_claimed && data.user_id && user && data.user_id !== user.id) {
  ...
  if (error || !session) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-gray-50 p-6 text-center">
        ...
        <p className="mt-2 text-gray-500 font-medium">{error || 'Terjadi kesalahan.'}</p>
        <button 
          onClick={() => router.push('/')}
